  ...
  useEffect(() => {
    ...
    socket.on("gameStarted", (data: any) => {
      ...
      currentPlayers.forEach((player) => {
        ...
        if (
          ...
          usePlayerStatesStore.setState((state) => ({
            players: state.players.filter((p) => p.state.id !== playerId),
          }));
        }
      });
      Object.values(clients).forEach((client: any) => {
        ...
        const isPlayerAlreadyAdded = currentPlayerIds.includes(playroomId);
        ...
        if (player && !isPlayerAlreadyAdded) {
          ...
          addPlayer(player, isMobile, id, vectorPosition);
        } else if (!player) {
          onPlayerJoin((newPlayer) => {
            console.log("newPlayer", newPlayer);
            if (newPlayer.id === playroomId) {
              addPlayer(newPlayer, isMobile, id, vectorPosition);
            }
          });
        }
      });
